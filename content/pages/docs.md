...
starter | bool | `false` | Is the card only not found in a booster pack? Can be `true` or `false`. (If false, it is usually omitted.) **Will be changed to `isStarter` in 4.2.2.**
...
isOnlineOnly | bool | `false` | Is the token only available online? Can be `true` or `false`. (If false, it is usually omitted.)
loyalty | string | `"7"` | Planeswalker loyalty value.
...
isFoilOnly | bool | `false` | Set available only in foil. (If false, it is usually omitted.)
...
tcgplayerGroupId | integer | 2199	| Group ID of the set on TCGPlayer.
...
AllSetFiles | | | | | [ZIP](json/AllSetFiles.zip) | Every set file. Useful for systems that can't load `AllSets.json`.
